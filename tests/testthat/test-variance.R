...
test_that("trendVar works on a basic scenario", {
    ...
    expect_equal(out$trend(mn-1), out$trend(mn)/mn * (mn-1))
...
test_that("trendVar works correctly on SCESet objects", {
    ...
    suppressWarnings(expect_error(trendVar(X, parametric=FALSE), "invalid 'x'")) # because there aren't any spike-ins for loess.
...
test_that("trendVar works with other trend functions",  {
    ...
    expect_equal(out.semi$trend(mn-1), out.semi$trend(mn)/mn * (mn-1))
    ...
    expect_equal(out.spl$trend(mn-1), out.spl$trend(mn)/mn * (mn-1))
})
...
expect_error(trendVar(d[0,,drop=FALSE], parametric=FALSE), "invalid 'x'") # loess fails with empty input vectors.
